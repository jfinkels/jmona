 ...
 */
public class MonaTreeMapping implements Function<Tree, BufferedImage> {
   ...
   */
  public MonaTreeMapping(final int initialWidth, final int initialHeight) {
   ...
   * 
   * This method assumes that the root of the Tree can be cast to
   * ColoredPolygonNode.
   * 
   * @param tree
   ...
   */
  @Override
  public BufferedImage execute(final Tree tree) throws MappingException {
    final ColoredPolygonNode root = (ColoredPolygonNode) tree.root();
    try {
      return this.monaMapping.execute(root.evaluate());
